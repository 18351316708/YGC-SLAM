...
// 设置YOLO 检测器
void Tracking::SetDetector(YOLO* pDetector)
{
    ...
    const double &timestamp)        //时间戳
{
    ...
    mCurrentFrame = Frame(
        ...
        mbf,                    //相机基线*相机焦距
        mThDepth);              //内外点区分深度阈值
    mCurrentFrame.mmDetectMap = mpDetector->dect;
    mCurrentFrame.mvDynamicArea = mpDetector->mvDynamicArea;
    // 步骤4：跟踪
    Track();
    mpDetector->dect.clear();
...
void Tracking::CreateNewKeyFrame()
{
    ...
    ///add
    pKF->mmDetectMap = mCurrentFrame.mmDetectMap;
    ...
    ///
    mpPointCloudMapping->insertKeyFrame( pKF, this->mImRGB, this->mImDepth,vpKFs,idk);
