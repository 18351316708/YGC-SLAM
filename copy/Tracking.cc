...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename,bool& isKeyframe)
{
    mImGray = imRGB;
    cv::Mat imDepth = imD;
    if(isYoloObject)
    ...
    if(isYoloObject)
    {
        mCurrentFrame.mmDetectMap = mpDetector->mmDetectMap;
