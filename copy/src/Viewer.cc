...
//查看器的主进程看来是外部函数所调用的
    void Viewer::Run() {
        ...
        pangolin::Var<bool> IsYoloObject("menu.IsYoloObject", false, true);
...
//            mpTracker->isYoloObject=false;
            ...
            for (auto vit = dect.begin(); vit != dect.end(); vit++) {
                cv::rectangle(im, vit->bbox, cv::Scalar(0, 0, 255), 2);
                //函数 cv::putText()：在图像中绘制制定文字
                draw_text(im, mClassnames[vit->class_idx], cv::Scalar(0, 0, 255), vit->bbox.tl(),false);
            }
...
void Viewer::draw_text(cv::Mat &img, const std::string &str,
                       const cv::Scalar &color, cv::Point pos, bool reverse) {
    auto t_size = cv::getTextSize(str, cv::FONT_HERSHEY_COMPLEX, 1, 1, nullptr);
    ...
    cv::putText(img, str, bottom_left, cv::FONT_HERSHEY_COMPLEX, 1, cv::Scalar(255, 255, 255) - color);
}
